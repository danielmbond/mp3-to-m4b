...
$START_PATH = "D:\books\Brad Thor\The Apostle"
$TEMP_PATH = "f:\temp"
$DELETE_OLD_MP3 = $true
...
$bookDirs = Get-ChildItem -Path $START_PATH -Recurse | Where-Object { $_.PSIsContainer }
...
foreach ($bookDir in $bookDirs) {
	...
	$command = ("ffmpeg -fflags +igndts -i `"$TEMP_PATH\$albumMp3`" -vn -c:a aac -q:a 1.3 -y `"$TEMP_PATH\$albumM4b`"").Replace("\\","\")
	$command
	Invoke-Expression $Command
	...
	(Get-ChildItem "$TEMP_PATH\$albumM4b") | set-title $album
	...
	if ($hasCoverArt -and (Test-Path $hasCoverArt)) {
		...
		Write-Host "Cover art is missing"
	}
}
